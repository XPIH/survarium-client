/**
 * Слушатель пакетов, которые зеркалирует роутер RouterOS инструментом Packet Sniffer
 * Имеется парсинг героев дня (если имеются все герои, в каждом уровне и категории)
 * Также имеется парсинг чата
 * 
 * Весь полученный трафик складывается в log.json
 */
...
function getTopOfTheDay(buffer) {
	...
	try {
		// могут отсутствовать произвольные позиции
		// и даже целые уровни
...
// в героях дня нет героев 1-4 уровней
`
...
// поиск игрока в окне добавления друга
`0000   90 9e 78 4e 05 04 07 76 61 73 65 6b 65 72        ..xN...vaseker`
}

function foundPlayers() {
0xAF;
// результат поиска игроков в окне добавления друга
...
0xAF;
// похоже на коллбек успешного добавления друга, но код тот же, что foundPlayers
`
